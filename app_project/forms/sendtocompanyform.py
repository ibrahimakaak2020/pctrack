...
from app.models.tables import CompanyUser, Equipment, User, Workshop
...
class MaintenanceRecordForm(FlaskForm):
    ...
    is_external = BooleanField('External Maintenance', validators=[DataRequired()])
    workshop_id = SelectField('Workshop', coerce=int, validators=[Optional()])
    ...
    current_status = SelectField('Current Status', choices=[('pending', 'Pending'), ('completed', 'Completed'), ('in_progress', 'In Progress')], validators=[DataRequired()])
